...
public class FileSelection {
  ...
  public FileSelection minusDirectoriesPartial(DrillFileSystem fs) throws IOException {
    assert !isExpandedFully();

    if (isExpandedPartial()) {
      return this;
    }
    Stopwatch timer = Stopwatch.createStarted();
    final List<FileStatus> statuses = getStatuses(fs);
    ...
    final List<FileStatus> dirs = fs.list(true, true /* dirs only */, paths);
    final FileSelection fileSel = create(dirs, null, selectionRoot);
    logger.debug("FileSelection.minusDirectories() took {} ms, numFiles: {}",
        timer.elapsed(TimeUnit.MILLISECONDS), total);

    // fileSel will be null if we query an empty folder
    if (fileSel != null) {
      fileSel.setExpandedPartial();
    }
  ...
  public boolean supportDirPrunig() {
    // currently we only support pruning if the directories have been expanded (this may change in the future)
